...
class StartState extends GameState {
    onEnter() {
        ...
        // Инициализация игрового уровня, персонажей и т.д.
        //для клавиатуры
...
class PlayState extends GameState {
    onEnter() {
        ...
        // Логика игры: управление персонажами, физика, обработка событий и т.д.
    }
    handleInput(input) {
        if (input === 'PAUSE') {
            return new PauseState();
        } else if (input === 'GAME_OVER') {
...
class PauseState extends GameState {
    onEnter() {
        ...
        // Остановка всех движений, анимаций и таймеров
    }
    handleInput(input) {
        if (input === 'RESUME') {
            return new PlayState();
...
class GameOverState extends GameState {
    onEnter() {
        console.log('Геймовер.');
        // Показываем экран геймовера, сохраняем прогресс и т.д.
    }
    handleInput(input) {
        if (input === 'RESTART') {
...
// Класс игры, управляющий состояниями
class Game {
    constructor() {
        this.state = new StartState();  // Начальное состояние игры
        this.state.onEnter();
    }
...
game.handleInput('PLAY');       // Игра продолжается.
game.handleInput('PAUSE');      // Игра на паузе.
game.handleInput('RESUME');     // Игра продолжается.
game.handleInput('GAME_OVER');  // Геймовер.
game.handleInput('RESTART');    // Игра началась.
